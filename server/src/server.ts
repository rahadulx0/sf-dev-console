...
app.post<{Body:{org:string,sobject:string,recordIds:string[],confirmation:string}}>('/api/data/records/delete', async (req) => { const org=safeOrg(req.body.org),sobject=safeType(req.body.sobject);const recordIds=[...new Set(req.body.recordIds||[])].slice(0,50).map(id=>safeId(id,'record ID'));if(!recordIds.length)throw new Error('Select at least one record');const expected=`DELETE ${recordIds.length} RECORDS FROM ${sobject}`;if(req.body.confirmation!==expected)throw new Error(`Confirmation must exactly match: ${expected}`);const deleted:string[]=[];const failed:{id:string,error:string}[]=[];for(const id of recordIds){try{await cli.execute(['data','delete','record','--sobject',sobject,'--record-id',id,'--target-org',org],{timeoutMs:120_000});deleted.push(id)}catch(e){failed.push({id,error:e instanceof Error?e.message:String(e)})}}return {deleted,failed}; });
app.get<{Params:{org:string};Querystring:{category?:string}}>('/api/orgs/:org/objects', async (req) => { const category=['all','standard','custom'].includes((req.query.category||'').toLowerCase())?req.query.category!.toLowerCase():'all';return { objects: await cli.execute(['sobject','list','--sobject',category,'--target-org',safeOrg(req.params.org)]) }; });
app.get<{Params:{org:string,name:string}}>('/api/orgs/:org/objects/:name', async (req) => ({ describe: await cli.execute(['sobject','describe','--sobject',safeType(req.params.name),'--target-org',safeOrg(req.params.org)],{timeoutMs:120_000}) }));
